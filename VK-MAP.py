...
# --------------------------------------------------
INDEX_FILE_ID = "1AED2LhxtbH-b3ORIGSKY95xAjeqsqlue"
...
def fetch_from_drive(file_id, save_path):
    ...
    return (
        ...
        and os.path.getsize(save_path) > 1024 * 1024
    )
...
def download_index_files(target_dir):
    ...
    # 1MB 以下のファイル（エラーHTML等）を除外する判定閾値
    MIN_SIZE = 1024 * 1024
    ...
    with st.spinner("📦 Downloading index files..."):
        if (
            ...
            or os.path.getsize(index_path) < MIN_SIZE
        ):
...
def load_system():
    ...
    # フォルダ名を更新して過去の破損キャッシュを回避
    cache_dir = os.path.join(BASE_DIR, "cache_vkmap_v3")
